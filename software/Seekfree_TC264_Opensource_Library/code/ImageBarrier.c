...
#define TOF_DISTANCE_THR 900//TOF测距的阈值
...
uint8 BarrierIdentify(void)
{
    ...
    switch(barrier_status)
    {
        case 0://识别路障
            ...
            if(dl1a_distance_mm <= TOF_DISTANCE_THR && dl1a_finsh_flag==1)//tof检测到路障
            {
                ...
                if(BarrierImageVerify()==1)
                {
                    ...
                    track_type=kTrackSpecial;image_bias=5;//左拐
                    encoder_dis_flag=1;
                    StartIntegralAngle_X(40);while(!icm_angle_x_flag);//等待40度
                    image_bias=0;
                    while(dis<575.11);encoder_dis_flag=0;
                    track_type=kTrackSpecial;image_bias=-7;//右拐
                    encoder_dis_flag=1;
                    StartIntegralAngle_X(70);while(!icm_angle_x_flag);//等待70度
                    image_bias=0;
                    while(dis<747.86);encoder_dis_flag=0;
                }
            }
            break;
        case 1://写死拐出去拐回来后，判断此时的左边线是不是真的左边线
            ...
            //验证右边线是否是赛道要走的左边
            else if(r_line_count>20 && (r_growth_direction[1]+r_growth_direction[0]-r_growth_direction[4]-r_growth_direction[3])>r_line_count-20 && R<300)
            {
